...
func (r *EventRepository) GetEventsByStatusV1(
	...
	switch status {
	case "open", "today":
		whereConditions = append(whereConditions, "e.start_time <= NOW() AND e.end_time >= NOW() AND e.status NOT IN ($1, $2)")
		queryArgs = append(queryArgs, "CLOSED", "CANCELLED")

	case "upcoming":
		whereConditions = append(whereConditions, "e.start_time > NOW() AND e.status NOT IN ($1, $2)")
		queryArgs = append(queryArgs, "CLOSED", "CANCELLED")

	case "past", "closed":
		whereConditions = append(whereConditions, "e.end_time < NOW() AND e.status NOT IN ($1, $2)")
		queryArgs = append(queryArgs, "CLOSED", "CANCELLED")
