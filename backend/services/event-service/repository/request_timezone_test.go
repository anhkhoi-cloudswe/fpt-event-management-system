...
// TestSetEventRequestTimeFields_PreservesPreferredWallClock verifies that
// preferred times keep the wall-clock values stored by the handler.
func TestSetEventRequestTimeFields_PreservesPreferredWallClock(t *testing.T) {
	tests := []struct {
		name         string
		dbWallClock  string
		expectedJSON string
		...
		{
			name:         "07:00 wall-clock remains 07:00 VN",
			dbWallClock:  "2026-04-01 07:00:00",
			expectedJSON: "2026-04-01T07:00:00+07:00",
			description:  "If user enters 07:00 VN, stored wall-clock should read back as 07:00 VN",
		},
		{
			name:         "09:00 wall-clock remains 09:00 VN",
			dbWallClock:  "2026-04-01 09:00:00",
			expectedJSON: "2026-04-01T09:00:00+07:00",
			description:  "If user enters 09:00 VN, stored wall-clock should read back as 09:00 VN",
		},
		{
			name:         "20:00 wall-clock remains 20:00 VN",
			dbWallClock:  "2026-04-01 20:00:00",
			expectedJSON: "2026-04-01T20:00:00+07:00",
			description:  "If user enters 20:00 VN, stored wall-clock should read back as 20:00 VN",
		},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			dbTime, _ := time.Parse("2006-01-02 15:04:05", tt.dbWallClock)
			...
			if *req.PreferredStartTime != tt.expectedJSON {
				t.Errorf("%s: preferred wall-clock mismatch\n  got:      %s\n  expected: %s\n  description: %s",
...
func TestRequestDetailJSON_NoDoubleOffsetInMarshal(t *testing.T) {
	// Preferred times are stored as Vietnam wall-clock values.
	wallClockTime := time.Date(2026, 4, 1, 9, 0, 0, 0, time.UTC)
	sqlNullTime := sql.NullTime{Time: wallClockTime, Valid: true}
...
// TestAllRequestTimePaths verifies preferred wall-clock fields and audit UTC fields.
func TestAllRequestTimePaths(t *testing.T) {
	preferredWallClock := sql.NullTime{Time: time.Date(2026, 4, 1, 9, 0, 0, 0, time.UTC), Valid: true}
	auditUTC := sql.NullTime{Time: time.Date(2026, 4, 1, 2, 0, 0, 0, time.UTC), Valid: true}
	...
	for _, tt := range tests {
		t.Run(tt.fieldName, func(t *testing.T) {
			req := &models.EventRequest{}
			setEventRequestTimeFields(req, preferredWallClock, preferredWallClock, auditUTC, auditUTC)
