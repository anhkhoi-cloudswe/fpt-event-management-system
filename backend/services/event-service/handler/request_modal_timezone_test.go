...
func TestRequestModalTimeflow_Smoke(t *testing.T) {
	// Construct a dynamic date exactly 2 days in the future to satisfy ValidateEventTime rules
	futureDate := utils.NowInVietnam().Add(48 * time.Hour)
	userInputStart := fmt.Sprintf("%04d-%02d-%02dT09:00:00", futureDate.Year(), futureDate.Month(), futureDate.Day())
	userInputEnd := fmt.Sprintf("%04d-%02d-%02dT10:00:00", futureDate.Year(), futureDate.Month(), futureDate.Day())
	...
	// ===== VERIFICATION =====
	expectedJSON := fmt.Sprintf("%04d-%02d-%02dT09:00:00+07:00", futureDate.Year(), futureDate.Month(), futureDate.Day())
