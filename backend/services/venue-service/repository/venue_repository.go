...
import (
	...
	"github.com/fpt-event-services/services/venue-service/models"
)
...
func (r *VenueRepository) GetVenueByID(ctx context.Context, venueID int) (*models.Venue, error) {
	query := `SELECT venue_id, venue_name, location, status FROM Venue WHERE venue_id = $1`
	...
	// Get areas
	areaQuery := `SELECT area_id, venue_id, area_name, floor, capacity, status FROM Venue_Area WHERE venue_id = $1 AND status != 'DELETED'`
...
func (r *VenueRepository) CreateVenue(ctx context.Context, req models.CreateVenueRequest) (int64, error) {
	query := `INSERT INTO Venue (venue_name, location, status) VALUES ($1, $2, 'AVAILABLE') RETURNING venue_id`

	var venueID int64
	err := r.db.QueryRowContext(ctx, query, req.VenueName, req.Location).Scan(&venueID)
	if err != nil {
...
func (r *VenueRepository) CheckVenueExists(ctx context.Context, venueID int) (bool, error) {
	query := `SELECT COUNT(*) as count FROM Venue WHERE venue_id = $1`
	var count int
...
func (r *VenueRepository) CheckDuplicateVenueName(ctx context.Context, venueName string, excludeVenueID int) (bool, error) {
	query := `SELECT COUNT(*) as count FROM Venue WHERE venue_name = $1 AND venue_id != $2 AND status != 'DELETED'`
	var count int
...
func (r *VenueRepository) UpdateVenue(ctx context.Context, req models.UpdateVenueRequest) error {
	...
	query := `UPDATE Venue SET venue_name = $1, location = $2 WHERE venue_id = $3`
...
func (r *VenueRepository) DeleteVenue(ctx context.Context, venueID int) error {
	query := `UPDATE Venue SET status = 'DELETED' WHERE venue_id = $1`
...
func (r *VenueRepository) GetAreasByVenueID(ctx context.Context, venueID int) ([]models.VenueArea, error) {
	query := `SELECT area_id, venue_id, area_name, floor, capacity, status FROM Venue_Area WHERE venue_id = $1 AND status != 'DELETED' ORDER BY area_id`
...
func (r *VenueRepository) CreateArea(ctx context.Context, req models.CreateAreaRequest) (int64, error) {
	query := `INSERT INTO Venue_Area (venue_id, area_name, floor, capacity, status) VALUES ($1, $2, $3, $4, 'AVAILABLE') RETURNING area_id`

	var areaID int64
	err := r.db.QueryRowContext(ctx, query, req.VenueID, req.AreaName, req.Floor, req.Capacity).Scan(&areaID)
	if err != nil {
...
func (r *VenueRepository) UpdateArea(ctx context.Context, req models.UpdateAreaRequest) error {
	query := `UPDATE Venue_Area SET area_name = $1, floor = $2, capacity = $3, status = $4 WHERE area_id = $5`
...
func (r *VenueRepository) DeleteArea(ctx context.Context, areaID int) error {
	query := `UPDATE Venue_Area SET status = 'DELETED' WHERE area_id = $1`
...
func (r *VenueRepository) GetSeatsForEvent(ctx context.Context, eventID int, seatType string) ([]models.Seat, error) {
	...
	query := `
		SELECT 
			...
			END AS seat_status
			FROM Seat s
			LEFT JOIN category_ticket ct ON s.category_ticket_id = ct.category_ticket_id AND ct.event_id = $2
			WHERE s.area_id = $3
			  AND s.status = 'ACTIVE'
	`
	...
	args := []interface{}{eventID, eventID, areaID}
	if seatType != "" {
...
func (r *VenueRepository) ReconfigureSeatsForEvent(ctx context.Context, tx *sql.Tx, eventID int, areaID int, vipCount int, standardCount int) error {
	...
	// 2. Xóa layout cũ
	_, err = tx.ExecContext(ctx, "DELETE FROM Event_Seat_Layout WHERE event_id = $1", eventID)
	if err != nil {
