...
func (r *ReportRepository) getReportDetailComposed(ctx context.Context, reportID int) (*models.ReportDetailStaffDTO, error) {
	...
	if processedBy.Valid {
		...
		staffErr := r.db.QueryRowContext(ctx, "SELECT COALESCE(full_name, 'Staff') FROM Users WHERE user_id = $1", processedBy.Int64).Scan(&staffName)
		if staffErr == nil {
	...
	if profileErr == nil && profileStatusCode == http.StatusOK {
		...
		r.db.QueryRowContext(ctx, "SELECT COALESCE(full_name, 'Khách hàng') FROM Users WHERE user_id = $1", userID).Scan(&dto.StudentName)
	}
	...
	if ticketErr == nil && ticketStatusCode == http.StatusOK {
		...
		r.db.QueryRowContext(ctx,
			`SELECT t.status, t.category_ticket_id, t.seat_id, ct.name, ct.price
			 ...
			 WHERE t.ticket_id = $1`, dto.TicketID,
...
func (r *ReportRepository) ListReportsForStaff(ctx context.Context, status string, page, pageSize int) ([]models.ReportListStaffDTO, error) {
	...
	args := []interface{}{}
	if status != "" {
		query += fmt.Sprintf(" WHERE r.status = $%d", len(args)+1)
		args = append(args, status)
	}

	query += fmt.Sprintf(" ORDER BY r.created_at DESC LIMIT $%d OFFSET $%d", len(args)+1, len(args)+2)
...
func (r *ReportRepository) ListReportsForStaffWithMetadata(ctx context.Context, status, search string, page, pageSize int) ([]models.ReportListStaffDTO, int, error) {
	...
	if search != "" {
		...
		conditions = append(conditions, fmt.Sprintf("(u.full_name LIKE $%d OR CAST(r.ticket_id AS VARCHAR) LIKE $%d)", len(args)+1, len(args)+2))
	...
	query := baseQuery + whereClause + fmt.Sprintf(" ORDER BY r.created_at DESC LIMIT $%d OFFSET $%d", len(args)+1, len(args)+2)
	args = append(args, pageSize, offset)
...
func (r *ReportRepository) processReportSaga(ctx context.Context, reportID, staffID int, approve bool, staffNote *string) (*ProcessReportResult, error) {
	...
	query := `SELECT user_id, ticket_id, status FROM Report WHERE report_id = $1 FOR UPDATE`
...
func (r *ReportRepository) processReportMonolith(ctx context.Context, reportID, staffID int, approve bool, staffNote *string) (*ProcessReportResult, error) {
	...
	query = `SELECT status FROM Ticket WHERE ticket_id = $1 FOR UPDATE`
	...
	err = tx.QueryRowContext(ctx, "SELECT wallet_id, balance FROM Wallet WHERE user_id = $1 FOR UPDATE", userID).Scan(&walletID, &balanceBefore)
	if err == sql.ErrNoRows {
	...
	_, txErr := tx.ExecContext(ctx,
		`INSERT INTO Wallet_Transaction (wallet_id, user_id, type, amount, balance_before, balance_after, reference_type, reference_id, description)
		 VALUES ($1, $2, 'CREDIT', $3, $4, $5, 'REFUND', $6, $7)`,
	...
	query = `UPDATE Ticket SET status = 'REFUNDED' WHERE ticket_id = $1 AND status = 'CHECKED_IN'`
