...
func (r *StaffRepository) updateTicketCheckinLocal(ctx context.Context, ticketID int) (int64, error) {
	...
	query := `UPDATE Ticket SET status = 'CHECKED_IN', checkin_time = NOW() WHERE ticket_id = $1 AND status = 'BOOKED'`
...
func (r *StaffRepository) updateTicketCheckoutLocal(ctx context.Context, ticketID int) (int64, error) {
	...
	query := `UPDATE Ticket SET status = 'CHECKED_OUT', check_out_time = NOW() WHERE ticket_id = $1 AND status = 'CHECKED_IN'`
...
func (r *StaffRepository) getReportsForStaffComposed(ctx context.Context) ([]models.ReportListResponse, error) {
	...
	for i := range reportRows {
		...
		if err == nil && sc == http.StatusOK {
			...
			// Fallback local
			r.db.QueryRowContext(ctx, "SELECT COALESCE(full_name, 'Khách hàng') FROM Users WHERE user_id = $1", rr.UserID).Scan(&rr.Report.StudentName)
		}
		...
		if err2 == nil && sc2 == http.StatusOK {
			...
			r.db.QueryRowContext(ctx,
				`SELECT t.status, ct.name, ct.price FROM Ticket t
				 ...
				 WHERE t.ticket_id = $1`, rr.TicketID,
...
func (r *StaffRepository) getReportDetailForStaffComposed(ctx context.Context, reportID int) (*models.ReportDetailResponse, error) {
	...
	localQuery := `SELECT report_id, user_id, ticket_id, title, description, image_url, created_at, status FROM Report WHERE report_id = $1`
	...
	if err == nil && sc == http.StatusOK {
		...
		r.db.QueryRowContext(ctx, "SELECT COALESCE(full_name, 'Khách hàng') FROM Users WHERE user_id = $1", userID).Scan(&report.StudentName)
	}
	...
	if report.SeatID != nil {
		...
		r.db.QueryRowContext(ctx,
			`SELECT s.seat_code, s.row_no, s.col_no,
			 ...
			 WHERE s.seat_id = $1`, *report.SeatID,
...
func (r *StaffRepository) UpdateSystemConfig(ctx context.Context, key, value string) error {
	...
	var count int
	checkQuery := `SELECT COUNT(*) FROM System_Config WHERE config_key = $1`
	...
	if count > 0 {
		...
		updateQuery := `UPDATE System_Config SET config_value = $1 WHERE config_key = $2`
		...
		// Insert new config
		insertQuery := `INSERT INTO System_Config (config_key, config_value) VALUES ($1, $2)`
...
func (r *StaffRepository) GetTicketIDByQRCode(ctx context.Context, qrCodeValue string) (int, error) {
	query := `SELECT ticket_id FROM Ticket WHERE qr_code_value = $1 LIMIT 1`
...
func (r *StaffRepository) VerifyEventOwnership(ctx context.Context, userID int, eventID int) (bool, error) {
	query := `
		SELECT COUNT(*) 
		FROM Event_Request 
		WHERE created_event_id = $1 AND requester_id = $2
	`
