...
func (h *TicketInternalHandler) HandleGetCategoryTicketInfo(ctx context.Context, request events.APIGatewayProxyRequest) (events.APIGatewayProxyResponse, error) {
	...
	query := `SELECT category_ticket_id, name, price, event_id FROM category_ticket WHERE category_ticket_id = $1`
...
func (h *TicketInternalHandler) HandleGetCategoryTicketsByEvent(ctx context.Context, request events.APIGatewayProxyRequest) (events.APIGatewayProxyResponse, error) {
	...
	query := `SELECT category_ticket_id, name, price, event_id FROM category_ticket WHERE event_id = $1`
...
func (h *TicketInternalHandler) HandleGetSeatStatuses(ctx context.Context, request events.APIGatewayProxyRequest) (events.APIGatewayProxyResponse, error) {
	...
	// Build IN clause for PostgreSQL ($2, $3, ...)
	...
	// Need eventID once (used as $1 in both subqueries), then seat IDs
	finalArgs := make([]interface{}, 0, len(seatIDs)+1)
	finalArgs = append(finalArgs, eventID)
...
func (h *TicketInternalHandler) HandleRefundTicket(ctx context.Context, request events.APIGatewayProxyRequest) (events.APIGatewayProxyResponse, error) {
	...
	err := h.db.QueryRowContext(ctx, "SELECT status FROM Ticket WHERE ticket_id = $1", req.TicketID).Scan(&currentStatus)
	if err != nil {
		if err == sql.ErrNoRows {
	...
	result, err := h.db.ExecContext(ctx,
		"UPDATE Ticket SET status = 'REFUNDED' WHERE ticket_id = $1 AND status = 'CHECKED_IN'",
		req.TicketID,
	)
	if err != nil {
...
func (h *TicketInternalHandler) HandleRevertRefund(ctx context.Context, request events.APIGatewayProxyRequest) (events.APIGatewayProxyResponse, error) {
	...
	result, err := h.db.ExecContext(ctx,
		"UPDATE Ticket SET status = $1 WHERE ticket_id = $2 AND status = 'REFUNDED'",
...
func (h *TicketInternalHandler) HandleCheckinTicket(ctx context.Context, request events.APIGatewayProxyRequest) (events.APIGatewayProxyResponse, error) {
	...
	result, err := h.db.ExecContext(ctx,
		"UPDATE Ticket SET status = 'CHECKED_IN', checkin_time = NOW() WHERE ticket_id = $1 AND status = 'BOOKED'",
		req.TicketID,
	)
	if err != nil {
...
func (h *TicketInternalHandler) HandleCheckoutTicket(ctx context.Context, request events.APIGatewayProxyRequest) (events.APIGatewayProxyResponse, error) {
	...
	result, err := h.db.ExecContext(ctx,
		"UPDATE Ticket SET status = 'CHECKED_OUT', check_out_time = NOW() WHERE ticket_id = $1 AND status = 'CHECKED_IN'",
		req.TicketID,
	)
	if err != nil {
...
type eventDetailSummary struct {
...
func (h *TicketInternalHandler) HandleRefundAllByEvent(ctx context.Context, request events.APIGatewayProxyRequest) (events.APIGatewayProxyResponse, error) {
	...
	eventName, err := h.resolveEventNameForRefund(ctx, req.EventID)
	if err != nil {
		h.logger.Warn("[TICKET] ❌ RefundAllByEvent: resolve event name failed for event %d: %v", req.EventID, err)
	...
	rows, err := h.db.QueryContext(ctx, `
		...
		WHERE t.event_id = $1 AND t.status IN ('BOOKED', 'PENDING', 'CHECKED_IN')
	`, req.EventID)
	if err != nil {
	...
	if err := rows.Err(); err != nil {
		h.logger.Warn("[TICKET] ❌ RefundAllByEvent: iterate ticket rows failed for event %d: %v", req.EventID, err)
		rows.Close()
		return createTicketInternalResponse(http.StatusInternalServerError, map[string]string{"error": "query iteration failed"})
	}
	rows.Close()

	refunded := 0
	var totalAmount float64
	userRefundMap := make(map[int]*UserRefundSummary)
	...
	for _, t := range tickets {
		...
		res, err := tx.ExecContext(ctx,
			"UPDATE ticket SET status = 'REFUNDED' WHERE ticket_id = $1 AND status IN ('BOOKED', 'PENDING', 'CHECKED_IN')",
			t.TicketID,
		)
		if err != nil {
			tx.Rollback()
		...
		tx.ExecContext(ctx, "INSERT INTO Wallet (user_id, balance) VALUES ($1, 0) ON CONFLICT (user_id) DO NOTHING", t.UserID)

		// Lock wallet row and get balance
		var walletID int
		var currentBalance float64
		err = tx.QueryRowContext(ctx,
			"SELECT wallet_id, balance FROM Wallet WHERE user_id = $1 FOR UPDATE", t.UserID,
		...
		_, _ = tx.ExecContext(ctx,
			`INSERT INTO Wallet_Transaction (wallet_id, user_id, type, amount, balance_before, balance_after, reference_type, reference_id, description)
			 VALUES ($1, $2, 'CREDIT', $3, $4, $5, 'TICKET', $6, $7)`,
		...
		if !exists {
			...
			userRefundMap[t.UserID] = summary
		}
		summary.TotalAmount += t.Price
		summary.TicketIDs = append(summary.TicketIDs, t.TicketID)
	}

	h.sendAggregatedRefundEmails(userRefundMap)
...
func (h *TicketInternalHandler) resolveEventNameForRefund(ctx context.Context, eventID int) (string, error) {
	var localTitle sql.NullString
	err := h.db.QueryRowContext(ctx, "SELECT title FROM Event WHERE event_id = $1", eventID).Scan(&localTitle)
	if err == nil && strings.TrimSpace(localTitle.String) != "" {
		return strings.TrimSpace(localTitle.String), nil
	}
	if err != nil && err != sql.ErrNoRows {
		h.logger.Warn("[TICKET] ⚠️ ResolveEventName: local Event query failed for event %d: %v", eventID, err)
	}

	baseURL := commonconfig.MustGetServiceURLWithFallback("Event", "EVENT_SERVICE_URL", 8082)
	detailURL := strings.TrimSuffix(baseURL, "/") + "/api/events/detail"

	client := utils.NewInternalClient()
	var detail eventDetailSummary
	statusCode, svcErr := client.GetJSON(ctx, detailURL, map[string]string{"id": strconv.Itoa(eventID)}, &detail)
	if svcErr != nil {
		return "", fmt.Errorf("call event-service failed: %w", svcErr)
	}
	if statusCode < 200 || statusCode >= 300 {
		return "", fmt.Errorf("event-service returned status %d", statusCode)
	}
...
func (h *TicketInternalHandler) sendAggregatedRefundEmails(userRefundMap map[int]*UserRefundSummary) {
	if len(userRefundMap) == 0 {
		return
	}
	...
	for userID, summary := range userRefundMap {
		...
		s := summary
		go func() {
			if err := h.sendRefundAggregationEmailWithRetry(uid, notifyURL, s); err != nil {
				h.logger.Warn("[TICKET] ⚠️ Failed to send aggregated refund email to user %d (%s): %v", uid, s.Email, err)
			}
		}()
	}
}
...
func (h *TicketInternalHandler) sendRefundAggregationEmailWithRetry(userID int, notifyURL string, summary *UserRefundSummary) error {
	...
	for attempt := 1; attempt <= maxAttempts; attempt++ {
		...
		lastErr = err
		if statusCode < 200 || statusCode >= 300 {
			lastErr = fmt.Errorf("status=%d, err=%v", statusCode, err)
		}

		if attempt < maxAttempts {
			h.logger.Warn("[TICKET] Attempt %d/%d failed to send aggregated refund email to user %d (%s): %v", attempt, maxAttempts, userID, summary.Email, lastErr)
