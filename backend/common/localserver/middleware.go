...
func WithJWTAuth(next LambdaHandler) LambdaHandler {
	return func(ctx context.Context, request events.APIGatewayProxyRequest) (events.APIGatewayProxyResponse, error) {
		stripSpoofableIdentityHeaders(request.Headers)
		path := request.Path
...
func stripSpoofableIdentityHeaders(headers map[string]string) {
	for key := range headers {
		if strings.EqualFold(key, "X-User-Id") ||
			strings.EqualFold(key, "X-User-Role") ||
			strings.EqualFold(key, "X-User-Email") {
			delete(headers, key)
		}
	}
}
