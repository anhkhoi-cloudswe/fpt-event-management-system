...
export default function Dashboard() {
  ...
  const openEventDetail = useCallback(async (eventId: number) => {
    ...
    setSearchParams(prev => {
      ...
      params.set('open_event', eventId.toString())
  ...
  const hasAutoOpenedRef = useRef<number | null>(null)



  useEffect(() => {

    const openEventIdStr = searchParams.get('open_event')

    if (openEventIdStr) {

      const eventId = parseInt(openEventIdStr, 10)

      if (!isNaN(eventId) && hasAutoOpenedRef.current !== eventId) {

        hasAutoOpenedRef.current = eventId
  ...
  }, [searchParams, openEventDetail, isDetailOpen])
            ...
            <>
              ...
              <div className="w-full grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6 mb-8">
                ...
                {displayedEvents.map((event) => {
                  ...
                  const isSoldOut = event.seatsBooked !== undefined && event.totalCapacity !== undefined && event.totalCapacity > 0 && event.seatsBooked >= event.totalCapacity
                    ...
                    <button
                      ...
                      className={`relative text-left block rounded-2xl overflow-hidden bg-white/70 dark:bg-slate-900/70 backdrop-blur-md h-full flex flex-col border-2 ${
                        ...
                        isSoldOut
                          ...
                          ? 'border-gray-200 dark:border-slate-800 opacity-75 cursor-not-allowed shadow-sm'

                          : showTodayBadge

                            ? 'border-red-500 shadow-xl shadow-red-500/20 transform scale-[1.02] hover:shadow-2xl hover:shadow-orange-500/10 hover:border-orange-500 hover:-translate-y-1.5 transition-all duration-500 cursor-pointer'
                    ...
                    >
                      ...
                      <div className="p-5 flex-1 flex flex-col">
                        ...
                        <div className="flex flex-wrap items-center gap-2 mb-3">
                          ...
                          <span className={`inline-block px-2.5 py-1 text-xs font-bold rounded-lg w-fit tracking-wide ${isEventClosedOrEnded(event, activeTab)
                          ...
                          {!isEventClosedOrEnded(event, activeTab) && getCountdownText(event.startTime) && (

                            <span className="inline-block px-2.5 py-1 text-xs font-extrabold rounded-lg w-fit tracking-wide bg-amber-100 text-amber-800 dark:bg-amber-950/60 dark:text-amber-300 animate-pulse">
                        ...
                        <div className="mt-4 pt-3 border-t border-gray-100 dark:border-slate-800 flex items-center justify-between">
                          ...
                          <p className="text-orange-600 dark:text-orange-500 text-xs font-bold tracking-wide">
