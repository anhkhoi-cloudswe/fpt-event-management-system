  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-60 backdrop-blur-sm p-4">
      <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-2xl dark:shadow-slate-950 max-w-md w-full overflow-hidden border border-slate-200 dark:border-slate-800 flex flex-col items-center p-6 text-center animate-fade-in">
        {/* Header */}
        <h3 className="text-xl font-bold text-slate-900 mb-1">Thanh toán chuyển khoản</h3>
        <p className="text-sm text-slate-500 mb-3">Quét mã VietQR để hoàn tất đặt vé</p>
        ...
        <div className="w-full bg-blue-50 bg-opacity-50 p-4 rounded-xl border border-blue-100 text-left text-sm space-y-2 mb-6">
          <div className="flex justify-between">
            ...
            <span className="font-semibold text-slate-800">{import.meta.env.VITE_BANK_NAME || 'MBBank'}</span>
          </div>
          <div className="flex justify-between">
            <span className="text-slate-500">Số tài khoản:</span>
            <span className="font-semibold text-slate-800">{import.meta.env.VITE_BANK_ACC || '2911121319'}</span>
          </div>
          <div className="flex justify-between">
        ...
        <div className="flex items-center justify-center space-x-3 mb-6">
          ...
          <span className="text-sm font-medium text-slate-700">Đang chờ bạn chuyển khoản chuyển trạng thái...</span>
        </div>
        ...
        <div className="w-full space-y-2">
          ...
          <button
            type="button"
            onClick={onCancel}
            className="w-full py-2.5 px-4 bg-gray-100 hover:bg-gray-200 text-slate-700 font-semibold rounded-lg transition-colors"
          >
