...
export function EventDetailModal({
  ...
  const text = {
    loading: currentLanguage === 'en' ? 'Loading event details...' : 'Đang tải chi tiết...',
    error: currentLanguage === 'en' ? 'Error' : 'Lỗi',
    copied: currentLanguage === 'en' ? 'Copied' : 'Đã copy',
    copyLink: currentLanguage === 'en' ? 'Copy Link' : 'Copy Link',
    eventPage: currentLanguage === 'en' ? 'Event Page' : 'Trang sự kiện',
    fullSize: currentLanguage === 'en' ? 'View full size' : 'Xem kích thước đầy đủ',
    area: currentLanguage === 'en' ? 'Area' : 'Khu vực',
    floor: currentLanguage === 'en' ? 'Floor' : 'Tầng',
    capacity: currentLanguage === 'en' ? 'Capacity' : 'Sức chứa',
    seats: currentLanguage === 'en' ? 'seats' : 'chỗ',
    registered: currentLanguage === 'en' ? 'Registered' : 'Đã đăng ký',
    people: currentLanguage === 'en' ? 'people' : 'người',
    description: currentLanguage === 'en' ? 'Event Description' : 'Mô tả sự kiện',
    speaker: currentLanguage === 'en' ? 'Speaker' : 'Diễn giả',
    ticketsSeats: currentLanguage === 'en' ? 'Tickets & Seats' : 'Vé & Đặt Ghế',
    ...
    remaining: currentLanguage === 'en' ? 'Remaining' : 'Còn lại',
    seatMap: currentLanguage === 'en' ? 'Seat Map' : 'Sơ đồ chọn ghế',
    selectedSeats: currentLanguage === 'en' ? 'Selected seats' : 'Ghế đã chọn',
    totalAmount: currentLanguage === 'en' ? 'Total amount' : 'Tổng số tiền',
    updateInfo: currentLanguage === 'en' ? 'Update info' : 'Cập nhật thông tin',
    close: currentLanguage === 'en' ? 'Close' : 'Đóng',
    register: currentLanguage === 'en' ? 'Register for Event' : 'Đăng ký sự kiện',
    chooseSeats: currentLanguage === 'en' ? 'Choose seats to register' : 'Chọn ghế để đăng ký',
    ongoing: currentLanguage === 'en' ? 'Event is ongoing' : 'Sự kiện đang diễn ra',
    seatPending: (seatCode: string) => currentLanguage === 'en'
      ? `Seat ${seatCode} is being held for payment. Please choose another seat.`
      : `Ghế ${seatCode} đang được giữ chỗ trong quá trình thanh toán. Vui lòng chọn ghế khác.`,
    noTicket: currentLanguage === 'en' ? 'No matching ticket type found' : 'Không tìm thấy loại vé phù hợp',
    detailMap: currentLanguage === 'en' ? 'event detail map' : 'sơ đồ chi tiết sự kiện',
  }
